...
def librosaextractFeatures(signal):
    ...
    frequency_coefficients = np.mean(np.fft.fftfreq(len(ft)))
    ...
    short_time_energy = np.mean(feature.rms(y=signal))
    average_zero_cross_rate = np.mean(feature.zero_crossing_rate(y=signal.astype('float')))
